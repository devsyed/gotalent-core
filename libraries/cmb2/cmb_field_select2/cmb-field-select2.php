...
class PW_CMB2_Field_Select2 {
	...
	public function get_pw_multiselect_options( $field_type_object, $field_escaped_value ) {
		...
		if ( !empty($options) && ! empty( $field_escaped_value ) && is_array($field_escaped_value) ) {
	...
	public function setup_admin_scripts() {
		...
		wp_enqueue_script( 'pw-select2-init', $asset_path . '/js/script.js', array( 'cmb2-scripts', 'wpfi-select2', 'jquery-ui-sortable' ), self::VERSION );
		wp_enqueue_style( 'pw-select2-tweaks', $asset_path . '/css/style.css', array( 'wpfi-select2' ), self::VERSION );
	}
}
