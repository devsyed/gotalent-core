...
class PW_CMB2_Field_Taxonomy_Select2_Search {
	...
	public function __construct() {
		...
		// Ajax endpoints.
		add_action( 'wpfi_ajax_wpjb_search_terms',  array($this, 'search_terms') );
	}
	...
	public function render_pw_taxonomy_select( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		echo $field_type_object->select( array(
			...
			'options'          => $this->get_pw_taxonomy_options( $field_type_object, $field_escaped_value ),
	 ...
	 */
	public function get_pw_taxonomy_options( $field_type_object, $field_escaped_value ) {
		$options = array();
	...
	public function setup_admin_scripts() {
		...
		wp_enqueue_script( 'pw-taxonomy-select2-loadmore-init', $asset_path . '/js/script.js', array( 'cmb2-scripts', 'wpfi-select2', 'jquery-ui-sortable' ), self::VERSION );
		wp_enqueue_style( 'pw-taxonomy-select2-loadmore-tweaks', $asset_path . '/css/style.css', array( 'wpfi-select2' ), self::VERSION );

		wp_localize_script( 'pw-taxonomy-select2-loadmore-init', 'wp_freeio_tax_search_opts', array(
    ...
    public function get_terms($taxonomy, $query_args = array(), $per_page = 0, $page = 1) {
        ...
        $defaults = array(
            ...
            'number' => $per_page,
            'offset' => $offset,
            'lang' => apply_filters( 'wp-freeio-current-lang', null )
	    );
	    $args = wp_parse_args( $query_args, $defaults );

	    $terms_hash = 'wpfi_cats_' . md5( wp_json_encode( $args ) . WP_Freeio_Cache_Helper::get_transient_version('wpfi_get_' . $taxonomy) );
